...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /*文章图片处理*/
    $article_img = $image->upload_image($_FILES['article_img']);
    if ($article_img == '')
    {
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
                "author_email, keywords, content, add_time, article_img, file_url, open_type, link, description) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
                ...
                "'$add_time', '$article_img', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]')";
    $db->query($sql);
...
if ($_REQUEST['act'] =='update')
{
    ...
    /*文章封面图片处理*/
    $sql = "SELECT article_img FROM " . $ecs->table('article') . " WHERE article_id = '$_POST[id]'";
    $old_img = $db->getOne($sql);
    $img = $image->upload_image($_FILES['article_img']); 
    if(empty($img)){
        $article_img = $old_img;
    }else{
        $article_img = $img;
    ...
    if ($exc->edit("title='$_POST[title]', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', article_img ='$article_img', file_url ='$file_url', open_type='$open_type', content='$_POST[FCKeditor1]', link='$_POST[link_url]', description = '$_POST[description]'", $_POST['id']))
    {
