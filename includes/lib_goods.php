...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, cat_desc,cat_img,parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, g.add_time, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
        $goods[$idx]['add_time']     = local_date($GLOBALS['_CFG']['date_format'], $row['add_time']);
